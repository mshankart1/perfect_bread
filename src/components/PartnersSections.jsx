  ...
  return (
    <div className="bg-primary py-4 text-white">
      <section className="container mx-auto px-4">
        <h2 className="heading mb-8 max-md:mb-6 max-sm:mb-4">PERFECT PARTNERS</h2>
        <div className="grid grid-cols-5 max-lg:grid-cols-4 max-md:grid-cols-3 my-10 gap-10 max-lg:gap-6">
          {partners.map((image, index) => (
            <div key={image.split('/').pop()} className="w-full bg-white aspect-[1.1/1] border border-black/50 shadow-lg p-4 max-sm:p-2 rounded-xl object-contain">
              <Image
                src={image}
                alt={image.split('/').pop()}
                width={500}
